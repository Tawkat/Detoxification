...
PROMPT_DICT = {
    "prompt_no_input": (
        "[INST] <<SYS>>\nYou are a toxic to non-toxic style transfer (detoxification) model. Given a toxic input, you should rewrite the toxic input into non-toxic version. The non-toxic output should not contain any form of toxic text. Also, You must preserve the original meaning as much as possible.\n<</SYS>>\n\n{toxic} [/INST] "
    ),
  "prompt_pipeline": "Rewrite the following toxic input into non-toxic version. You must preserve the original meaning as much as possible.\n Input: "
}
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, debugging: bool):
        ...
        print(sources[0])
        targets = [f"Detoxification: {example['non_toxic']}{tokenizer.eos_token}" for example in list_data_dict]
